...
set output "AverageSRIHaplotypes.png"
set ylabel "Average Haplotype SRI" font ",20"
set xlabel "Years" font ",20"
set xtics font ",15"
set ytics font ",15"
set yr [0:6]
#set label "Decoy Presenting Virus" font ",20" at 70000,9
plot "SIhapAverage_L=1.txt" every 100 using 1:2 w l ls 1 title "Rec (%)=100" ,\
"SIhapAverage_L=2.txt" every 100 using 1:2 w l ls 2 title "Rec (%)=96" ,\
"SIhapAverage_L=3.txt" every 100 using 1:2 w l ls 3 title "Rec (%)=70" ,\
"SIhapAverage_L=4.txt" every 100 using 1:2 w l ls 4 title "Rec (%)=40" ,\
"SIhapAverage_L=5.txt" every 100 using 1:2 w l ls 5 title "Rec (%)=20" ,\
"SIhapAverage_L=6.txt" every 100 using 1:2 w l ls 6 title "Rec (%)=9.8" ,\
"SIhapAverage_L=7.txt" every 100 using 1:2 w l ls 7 title "Rec (%)=4.1" ,\
"SIhapAverage_L=8.txt" every 100 using 1:2 w l ls 8 title "Rec (%)=2.4" ,\
"SIhapAverage_L=9.txt" every 100 using 1:2 w l ls 9 title "Rec (%)=0.1" ,\
"SIhapAverage_L=10.txt" every 100 using 1:2 w l ls 10 title "Rec (%)=0.06" 
