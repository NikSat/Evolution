...
set output "AverageSRI.png"
set ylabel "Average SRI" font ",20"
set xlabel "Years" font ",20"
set xtics font ",15"
set ytics font ",15"
set yr [4:12]
#set label "Decoy Presenting Virus" font ",20" at 70000,9
plot "SIAverage_L=1.txt" every 100 using 1:2 w l ls 1 title "Rec (%)=100" ,\
"SIAverage_L=2.txt" every 100 using 1:2 w l ls 2 title "Rec (%)=96" ,\
"SIAverage_L=3.txt" every 100 using 1:2 w l ls 3 title "Rec (%)=70" ,\
"SIAverage_L=4.txt" every 100 using 1:2 w l ls 4 title "Rec (%)=40" ,\
"SIAverage_L=5.txt" every 100 using 1:2 w l ls 5 title "Rec (%)=20" ,\
"SIAverage_L=6.txt" every 100 using 1:2 w l ls 6 title "Rec (%)=9.8" ,\
"SIAverage_L=7.txt" every 100 using 1:2 w l ls 7 title "Rec (%)=4.1" ,\
"SIAverage_L=8.txt" every 100 using 1:2 w l ls 8 title "Rec (%)=2.4" ,\
"SIAverage_L=9.txt" every 100 using 1:2 w l ls 9 title "Rec (%)=0.1" ,\
"SIAverage_L=10.txt" every 100 using 1:2 w l ls 10 title "Rec (%)=0.06" 
